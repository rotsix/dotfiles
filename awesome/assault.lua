...
function assault.fit (assault, width, height)
	local width = 2 + data[assault].width + (data[assault].stroke_width * 2) + 
data[assault].peg_width
...
function assault.draw (assault, wibox, cr, width, height)
	local center_x = (width / 2.0) - ((data[assault].width + (data[assault].stroke_width * 2)) 
/ 2.0)
	local center_y = (height / 2.0) - ((data[assault].height + (data[assault].stroke_width * 
2)) / 2.0)
	...
	if acpi_is_on_ac_power(data[assault].adapter) then
		...
		cr:append_path(battery_bolt_generate(data[assault].bolt_width, 
data[assault].bolt_height))
		cr:translate(-bolt_x, -bolt_y)
...
function assault.new (args)
	...
	local peg_top = args.peg_top or (((height + (stroke_width * 2)) / 2.0) - (peg_height / 
2.0))
